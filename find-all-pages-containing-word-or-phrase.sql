...
SET @stmt_one = CONCAT('SELECT DISTINCT post.id, post.post_title FROM ', @database, '.', @table_prefix, 'posts AS post WHERE post.post_content LIKE "', @expression, '"\n');
...
SET @stmt_one = CONCAT('SELECT DISTINCT post.id, post.post_title FROM ', @database, '.', @table_prefix, 'postmeta AS pm\n');
