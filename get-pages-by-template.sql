...
SET @stmt_one = CONCAT('SELECT * FROM ', @database, '.', @table_prefix, 'posts as posts\n');
SET @stmt_two = CONCAT('INNER JOIN ', @database, '.', @table_prefix, 'postmeta pm ON posts.id = pm.post_id\n');
